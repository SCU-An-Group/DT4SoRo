...
base_path = r'F:/Fast-pn-actuator/Fpn_bending/Board_new_new'
output_folder = os.path.join(base_path, 'output_data')

# 读取参数文件，构造参数矩阵 X 与 Xpred
namespace = {}
with open(base_path + r"/Parameters_P_detaH_new_new.py", "r") as file:  # 文件名修改
...
Xpred = np.array(newPara).reshape(1, -1).T
...
for j in range(num_test):
    ...
    file_name = 'Y_anyPara_pred_all_{}.csv'.format(param_str)

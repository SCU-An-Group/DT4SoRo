...
P = 0.01
detaH = 20.0
# execfile("Parameters_P_detaH.py")
# execfile("Parameters_for_Fpn_bending_Board.py")
...
mdb.models['Model-1'].Gravity(comp1=9810.0, createStepName='Step-Gravity', 
...
mdb.models['Model-1'].ImplicitDynamicsStep(initialInc=1e-02, maxNumInc=10000, minInc=
    1e-7, maxInc=1e-01, alpha=DEFAULT, amplitude=RAMP, application=QUASI_STATIC,
    initialConditions=OFF, name='Step-Pressure', nlgeom=ON, nohaf=OFF, previous='Step-Gravity')
...
mdb.models['Model-1'].rootAssembly.CoincidentPoint(fixedPoint=
    mdb.models['Model-1'].rootAssembly.datums[1].origin, movablePoint=
    mdb.models['Model-1'].rootAssembly.instances['Merged-1'].vertices.findAt((
    -65.928617, -7.036239, 99.0), ))
# create Board
mdb.models['Model-1'].ConstrainedSketch(name='__profile__', sheetSize=200.0)
mdb.models['Model-1'].sketches['__profile__'].rectangle(point1=(85.0, 0.0), 
    point2=(115.0, 30.0))
mdb.models['Model-1'].Part(dimensionality=THREE_D, name='Board', type=
    DEFORMABLE_BODY)
mdb.models['Model-1'].parts['Board'].BaseSolidExtrude(depth=30.0, sketch=
    mdb.models['Model-1'].sketches['__profile__'])
del mdb.models['Model-1'].sketches['__profile__']
mdb.models['Model-1'].parts['Board'].Set(cells=
    mdb.models['Model-1'].parts['Board'].cells, name='Set-Board')
mdb.models['Model-1'].parts['Board'].Set(faces=
    mdb.models['Model-1'].parts['Board'].faces.findAt(((90.0, 16.666667, 0.0), 
    )), name='Set-BoardFace')
mdb.models['Model-1'].parts['Board'].Set(faces=
    mdb.models['Model-1'].parts['Board'].faces.findAt(((115.0, 16.666667, 15.0), 
    )), name='Set-BoardBC')
mdb.models['Model-1'].parts['Board'].Surface(name='Surf-BoardFace', side1Faces=
    mdb.models['Model-1'].parts['Board'].faces.findAt(((90.0, 16.666667, 0.0), 
    )))
# define material
mdb.models['Model-1'].Material(name='Board')
mdb.models['Model-1'].materials['Board'].Density(table=((7.85e-9, ), ))
mdb.models['Model-1'].materials['Board'].Elastic(table=((2.1e5, 0.3), ))
# create and define section
mdb.models['Model-1'].HomogeneousSolidSection(material='Board', name=
    'Sec-Board', thickness=None)
mdb.models['Model-1'].parts['Board'].SectionAssignment(offset=0.0, offsetField=
    '', offsetType=MIDDLE_SURFACE, region=Region(
    cells=mdb.models['Model-1'].parts['Board'].cells),
    sectionName='Sec-Board', thicknessAssignment=FROM_SECTION)
# mesh board
mdb.models['Model-1'].parts['Board'].setElementType(elemTypes=(ElemType(
    elemCode=C3D20R, elemLibrary=STANDARD), ElemType(elemCode=C3D20R, 
    elemLibrary=STANDARD), ElemType(elemCode=C3D20R, elemLibrary=STANDARD)), 
    regions=(mdb.models['Model-1'].parts['Board'].cells,))
mdb.models['Model-1'].parts['Board'].seedPart(deviationFactor=0.1, 
    minSizeFactor=0.1, size=Meshsize)
mdb.models['Model-1'].parts['Board'].generateMesh()
# move Board
mdb.models['Model-1'].rootAssembly.Instance(dependent=ON, name='Board-1', part=
    mdb.models['Model-1'].parts['Board'])
mdb.models['Model-1'].rootAssembly.rotate(angle=90.0, axisDirection=(10.0, 0.0, 
    0.0), axisPoint=(0.0, 0.0, 0.0), instanceList=('Board-1', ))
mdb.models['Model-1'].rootAssembly.translate(instanceList=('Board-1', ), 
    vector=(0.0, 0.0, 1.0))
mdb.models['Model-1'].rootAssembly.translate(instanceList=('Board-1', ), 
    vector=(0.0, -detaH, 0.0))
# define rigid board
RP_RF = mdb.models['Model-1'].rootAssembly.ReferencePoint(point=
    mdb.models['Model-1'].rootAssembly.instances['Merged-1'].vertices.findAt((
    6.0, 19.0, 26.0), ))
mdb.models['Model-1'].rootAssembly.Set(name='Set-RP-RF', referencePoints=(
    mdb.models['Model-1'].rootAssembly.referencePoints[RP_RF.id], ))
mdb.models['Model-1'].RigidBody(bodyRegion=
    mdb.models['Model-1'].rootAssembly.instances['Board-1'].sets['Set-Board'], 
    name='RigidBoard', refPointRegion=Region(referencePoints=(
    mdb.models['Model-1'].rootAssembly.referencePoints[RP_RF.id], )))
# define contact
mdb.models['Model-1'].ContactProperty('IntProp-BoardContact')
mdb.models['Model-1'].interactionProperties['IntProp-BoardContact'].TangentialBehavior(
    formulation=FRICTIONLESS)
mdb.models['Model-1'].interactionProperties['IntProp-BoardContact'].NormalBehavior(
    allowSeparation=ON, constraintEnforcementMethod=DEFAULT, 
    pressureOverclosure=HARD)
mdb.models['Model-1'].ContactStd(createStepName='Initial', name='Int-BoardContact')
mdb.models['Model-1'].interactions['Int-BoardContact'].includedPairs.setValuesInStep(
    stepName='Initial', useAllstar=ON)
mdb.models['Model-1'].interactions['Int-BoardContact'].contactPropertyAssignments.appendInStep(
    assignments=((GLOBAL, SELF, 'IntProp-BoardContact'), ), stepName='Initial')
# defien BC
mdb.models['Model-1'].rootAssembly.regenerate()
mdb.models['Model-1'].EncastreBC(createStepName='Initial', localCsys=None, 
    name='BC-3', region=
    mdb.models['Model-1'].rootAssembly.instances['Board-1'].sets['Set-BoardBC'])
# ODB output
mdb.models['Model-1'].FieldOutputRequest(createStepName='Step-Pressure', name=
    'F-Output-2', timeInterval=0.05, rebar=EXCLUDE, region=
    mdb.models['Model-1'].rootAssembly.sets['Set-RP-RF'], sectionPoints=DEFAULT
    , variables=('RF', ))
mdb.models['Model-1'].HistoryOutputRequest(createStepName='Step-Pressure', 
    name='H-Output-4', region=MODEL, rebar=EXCLUDE,sectionPoints=DEFAULT,
    variables=('CFNM', ))
# define history-output:COORD
mdb.models['Model-1'].rootAssembly.Set(name='Set-Board', nodes=
    mdb.models['Model-1'].rootAssembly.instances['Board-1'].nodes)
mdb.models['Model-1'].HistoryOutputRequest(createStepName='Step-Pressure',
    name='H-Output-5', rebar=EXCLUDE, region=
    mdb.models['Model-1'].rootAssembly.sets['Set-Board'], sectionPoints=DEFAULT, 
    variables=('COOR1', 'COOR2', 'COOR3'))
mdb.models['Model-1'].fieldOutputRequests['F-Output-1'].setValuesInStep(
    stepName='Step-Pressure', timeInterval=0.05)
...
jobName = 'Fpn_bending_Board' + '_P_' + str(P).replace('.', '') + '_detaH_' + str(detaH).replace('.', '')
menuName = 'Fpn_bending_Board'
DirName = menuName + '_P_' + str(P) + '_detaH_' + str(detaH)
rf_set_name = 'SET-RP-RF'
...
# if P in test_P_values and detaH in test_detaH_values:
...
# def export_board_node_data(odb):
#     output_filename = os.path.join(csv_dir, DirName + '_board_nodes.csv')
#     with open(output_filename, 'wb') as outfile:
#         writer = csv.writer(outfile)
#         writer.writerow(['Node Label', 'X', 'Y', 'Z'])
        
#         try:
#             board_instance = odb.rootAssembly.instances['BOARD-1']
#             for node in board_instance.nodes:
#                 node_label = node.label
#                 try:
#                     region = odb.steps[stepName].historyRegions['Node BOARD-1.%d' % node_label]
#                     X = region.historyOutputs['COOR1'].data[-1][1]
#                     Y = region.historyOutputs['COOR2'].data[-1][1]
#                     Z = region.historyOutputs['COOR3'].data[-1][1]
#                 except KeyError:
#                     X, Y, Z = node.coordinates
#                 writer.writerow([node_label, X, Y, Z])
                
#         except KeyError:
#             print("Error: Board-1 instance not found in ODB")
#             raise

# def export_board_element_data(odb):
#     output_filename = os.path.join(csv_dir, DirName + '_board_elements.csv')
#     with open(output_filename, 'wb') as outfile:
#         writer = csv.writer(outfile)
#         c3d20r_nodes = 20
#         header = ['Element Label'] + ['Node %d' % (i+1) for i in range(c3d20r_nodes)]
#         writer.writerow(header)
        
#         try:
#             board_instance = odb.rootAssembly.instances['BOARD-1']
#             for element in board_instance.elements:
#                 if element.type != 'C3D20R':
#                     continue
#                 connectivity = list(element.connectivity)
#                 if len(connectivity) < c3d20r_nodes:
#                     connectivity += [None]*(c3d20r_nodes - len(connectivity))
                
#                 writer.writerow([element.label] + connectivity)
...
#     export_board_node_data(odb)
#     export_board_element_data(odb)
...
# # output set RF data
# def get_unique_nodal_data(field_output, node_set):
#     unique_values = {}
#     for value in field_output.values:
#         node_label = value.nodeLabel
#         unique_values[node_label] = value 
#     return unique_values.values()

# odbpath=jobName + '.odb'
# target_set = odb.rootAssembly.nodeSets['SET-RP-RF']
# reaction_file = os.path.join(csv_dir, DirName + '_reaction_forces.csv')
# with open(reaction_file, 'w') as f:
#     f.write('Time,RF-Magnitude\n')
...
#         except KeyError:
#             print("Warning: RF field missing at time={}".format(current_time))
#             continue
        
#         subset = rf_field.getSubset(region=target_set)
#         unique_values = get_unique_nodal_data(subset, target_set)
        
#         total_force = 0.0
#         for value in unique_values:
#             rf_data = value.data
#             magnitude = math.sqrt(rf_data[0]**2 + rf_data[1]**2 + rf_data[2]**2)
#             total_force += magnitude
        
#         f.write("{0:.6f},{1:.6e}\n".format(current_time, total_force))
...
# REACTION_NODES = [101, 685, 686, 687, 688, 689, 690, 691, 692, 693, 694, 103]

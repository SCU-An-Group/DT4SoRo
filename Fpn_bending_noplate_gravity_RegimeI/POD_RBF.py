...
for dtype in data_types:
    ...
    Y_test_file = os.path.join(output_folder, 'Y_testing_{}.csv'.format(dtype))
...
for j in range(Xtest.shape[1]):
    ...
    file_name = 'Y_prediction_all_{}.csv'.format(test_P_values[j])
    output_path = os.path.join(output_folder, file_name)
    # 使用 np.savetxt 写入 CSV 文件，同时写入 header（设置 comments='' 以避免行首出现#）
    np.savetxt(output_path, combined, delimiter=',', fmt='%f',
               header=",".join(header), comments='')
    print("Combined prediction for {} saved to {}".format(test_P_values[j], output_path))